(function () {
    ...
    var tetris = {
      ...
      // Add this function to initialize and update high score from local storage
      initHighScore: function () {
        // Retrieve high score from local storage, or default to 0 if not present
        var storedHighScore = localStorage.getItem('tetrisHighScore');
        ...
        document.getElementById('highscore').querySelector('span').innerHTML = storedHighScore;
        this.highScore = storedHighScore; // Store high score in game object for reference
      },
      ...
      updateHighScore: function () {
        if (this.score > this.highScore) {
          ...
          document.getElementById('highscore').querySelector('span').innerHTML = this.highScore;
        }
      },
      initShapes: function () {
        this.curSqs = [];
      ...
      // تابع جدید برای حرکت با تأخیر
      delayedMove: function(dir) {
        var me = this;
        var delay = Math.max(500,(Math.random() * 1000)); // تولید تاخیر تصادفی
        setTimeout(function() {
          me.move(dir);
        }, delay);
      },

      // تابع اصلی برای پردازش کلیدهای فشرده شده توسط کاربر
      handleKey: function(e) {
        ...
        if(delayErrorEnable){
          switch (c) {
            case 37:
              this.delayedMove('L'); // تاخیر در حرکت به چپ
              break;
            case 38:
              this.delayedMove('RT'); // تاخیر در چرخش
              break;
            case 39:
              this.delayedMove('R'); // تاخیر در حرکت به راست
              break;
            case 40:
              this.delayedMove('D'); // تاخیر در حرکت به پایین
              break;
            case 27: //esc: pause
              this.togglePause();
              break;
      ...
      updateHighScore: function () {
        var highScoreSpan = document.getElementById('highscore-value');
        highScoreSpan.innerHTML = this.highScore;
      },
      gameOver: function () {
        this.clearTimers();
        isStart = false;
