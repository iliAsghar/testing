...
// ---------------------------- disruption ---------------------------- //
// شبیه‌سازی تاخیر (Latency)


console.log(state.bug.timeout);
const introduceLatency = () => {
    state.bug.timeout = true
        // تنظیم زمانی برای برطرف کردن اختلال پس از یک مدت زمان مشخص (مثلا 5 ثانیه)
    setTimeout(() => {
        state.bug.timeout = false;
    }, 5000); // زمان انتظار بر حسب میلی‌ثانیه
};
...
// ------------------ get disruption from backend ------------------ //
const disruptionType = window.disruption; // دریافت نوع اختلال از سمت سرور
console.log(disruptionType);
const setDisruption = () => {
    console.log('game over')
    const score = state.game.score;

    const applyDisruption = (callback) => {
        console.log(disruptionType);
        if (disruptionType === 'latency') {
            introduceLatency(callback, 1000); // تاخیر 1 ثانیه‌ای

        } else {
            callback(); // بدون اختلال
        }
    };
    ...
    applyDisruption(() => {
        /*sendScore(score)
            ...
            .then(data => {
                if (data.success) {
                    // Redirect to the new path after score is saved
                    window.location.href = '/user/survey/flappy-bird';
                } else {
...
const sendScore = (score) => {
    const gameId = 2; // ثابت بودن game_id با مقدار 2
    return fetch('/user/submit-score', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
        body: JSON.stringify({ score: score, game_id: gameId, disruption_type: disruptionType }) // JSON encoding the score and game_id
    });
};
...
const handleGameOver = () => {
    sendScore(state.game.score)
        .then(response => {
            if (!response.ok) {
                // در صورت خطا، متن پاسخ را برگرداند
                return response.text().then(text => { throw new Error(text) });
            }
            return response.json();
        })
...
// Original endGame function updated to call handleGameOver
function endGame() {
...
// Example of how to call endGame when game over occurs
const gameOver = () => {
    // Perform any other game over logic here...
    endGame();
};
            ...
            if (distance <= targetHitRadius) {
                ...
                if (!target.hit) {
                    ...
                    if (pointerSpeedScaled > minPointerSpeed) {
                        ...
                        if (target.health <= 0) {
                            document.getElementById("cutEffect").currentTime = 0;
                            document.getElementById("cutEffect").play()
...
function setupCanvases() {
    ...
    function frameHandler(timestamp) {
        console.log('state.bug.timeout ===>', state.bug.timeout)
        if (state.bug.timeout) return setTimeout(() => {
            frameHandler(timestamp)
            state.bug.timeout = false
        }, state.bug.timeoutValue);
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    const createCoverElement = () => {
        ...
        console.log('Cover element added:', cover);
    };

    // remove cover
    const removeCoverElement = () => {
        const cover = document.getElementById('interaction-blocker');
        if (cover) {
            document.body.removeChild(cover);
        }
    };

    // disable input for a set time (0.5s - 1s)
    const disableInput = () => {
        let timeoutDuration = Math.max(500, (Math.random() * 1000))
        if (disruptionType == 'touchNoise') {
            createCoverElement();
        }
        setTimeout(() => {
            removeCoverElement();
